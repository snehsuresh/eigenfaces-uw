...
folder_names = dataset_obj.folder_name_as_array
...
if algo_type == "pca":
    cv2.imshow("Original Image" , cv2.resize(np.array(np.reshape(images_matrix[:,1],[img_height, img_width]), dtype = np.uint8),(200, 200)))
    cv2.waitKey()
else:
